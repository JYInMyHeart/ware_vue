    ...
    <div class="app-container">
      <div class="box">
        <div class="filter-container">
          <el-input placeholder="销售商名称" v-model="pagination.queryString" style="width: 10%;"
        ...
        <el-table size="small" current-row-key="id" :data="dataList" stripe highlight-current-row>
          ...
          <el-table-column prop="name" label="销售商名称" align="center"></el-table-column>
        ...
        <!-- 新增标签弹层 -->
        <div class="add-form">
          <el-dialog title="新增销售商" :visible.sync="dialogFormVisible">
            <template>
...
export default {
  ...
  methods: {
    ...
    handleEdit () {
      // 发送请求，提交表单数据
      this.$axios.post('/dealer/web/update', this.formData,
    ...
    handleAdd () {
      ...
      this.$axios.post('/dealer/web/insert', this.formData,
        {
          headers: {
    ...
    handleUpdate (row) {
      ...
      // 发送请求，根据ID获取销售商数据
      this.$axios.get('/dealer/web/selectById?id=' + row.id,
        {
          headers: {
            'content-type': 'application/json',
            Authorization: this.Authorization
        ...
        if (response.data.code === 200) {
          ...
          this.activeName = 'first'
          // 把数据赋值给formData
          this.formData = response.data.data.dealer
        } else {
          this.$message.error(response.data.message)
        }
      })
    ...
    handleDelete (row) {
      this.$confirm('确认删除当前选中的记录吗?', '提示', { type: 'warning' }).then(() => {
        this.$axios.get('/dealer/web/delete?id=' + row.id,
          {
            headers: {
              'content-type': 'application/json',
              Authorization: this.Authorization
